function f(x) {
    return Math.exp(x)/(Math.log(1/(x-3))) === Infinity ? NaN : Math.exp(x)/(Math.log(1/(x-3)));
}

function maxArr(arr) { //диапазон осей
    let max = arr[0];
    let n = arr.length;
    for (let i = 0; i < n; i++) {
        if (!isNaN(arr[i]) && arr[i] > max || isNaN(max)) {
            max = +arr[i];
        }
    }
    return max;
...
function minArr(arr) {
    ...
    let n = arr.length;
    for (let i = 0; i < n; i++) {
        if (!isNaN(arr[i]) && arr[i] < min || isNaN(min)) {
            min = +arr[i];
        }
    }
    return min;
